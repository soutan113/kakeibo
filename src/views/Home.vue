...
<div>
  <v-card>
    <v-card-title>
      <!-- 月選択 -->
      <v-col cols="8">
        <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :return-value.sync="yearMonth" transition="scale-transition" offset-y max-width="290px" min-width="290px">
          <template v-slot:activator="{ on }">
            <v-text-field v-model="yearMonth" prepend-icon="mdi-calendar" readonly v-on="on" hide-details />
          </template>
          <v-date-picker v-model="yearMonth" type="month" color="green" locale="ja-jp" no-title scrollable>
            <v-spacer />
    ...
    <v-data-table
      ...
      <!-- 悠太列 -->
      <template v-slot:item.yuta="{ item }">
        {{ separate(item.yuta) }}
      </template>
      <!-- 佳菜子列 -->
      <template v-slot:item.kana="{ item }">
        {{ separate(item.kana) }}
      </template>
      <!-- 操作列 -->
